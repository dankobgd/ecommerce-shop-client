...
export function BrandLogoUploadField({ name }) {
  ...
  const thumbs = files.map(file => (
    <div style={thumbStyle} key={file.name}>
      <div style={thumbInnerStyle}>
        <img alt={`brand-preview-${file.name}`} src={file.preview} style={thumbImgStyle} />
      </div>
    </div>
  ));
  ...
  return (
    <div style={{ marginTop: '1rem' }}>
      <FormDropzone name={name} accept='image/*' onDrop={onDrop}>
        <Button variant='contained'>Upload Brand Thumbnail</Button>
      </FormDropzone>

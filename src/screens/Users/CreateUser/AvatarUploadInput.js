...
export function AvatarUploadInput({ name }) {
  ...
  const thumbs = files.map(file => (
    <div style={thumbStyle} key={file.name}>
      <div style={thumbInnerStyle}>
        <img alt={`user-avatar-preview-${file.name}`} src={file.preview} style={thumbImgStyle} />
      </div>
    </div>
  ));
  ...
  return (
    <div style={{ marginTop: '1rem' }}>
      <FormDropzone name={name} accept='image/*' onDrop={onDrop}>
        <Button variant='contained'>Choose User Avatar</Button>
      </FormDropzone>
